...
|---|---|---|---|
| **ARM64** | RB5009, CCR2004, CCR2116, CCR2216, hEX S (v7 refreshed) | High single-core IPC, native container support (`/container`), wire-speed WireGuard crypto. | High memory utilization if containerized microservices leak RAM. |
| **ARM32** | hAP ax2, hAP ax3, Chateau series | Excellent wireless AX offloading, balanced SOHO performance. | Limited RAM (often 512MB-1GB); running Docker containers will strain flash NAND. |
| **MIPSBE** | hEX (RB750Gr3), RB2011, RB3011, cAP ac | Inexpensive, low power consumption, reliable for simple NAT. | **Cannot run CAKE QoS above 150-250 Mbps** without hitting 100% CPU; no container support. |
